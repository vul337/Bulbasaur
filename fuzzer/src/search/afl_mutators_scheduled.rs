...
use super::*;
use rand::Rng;
use bulbasaur_common::utils::choose_start_point;
...
pub fn mut_bitflip(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    let len = buf.len();
    if len == 0 {
        return;
    }
    ...
    let off = choose_start_point(len, position, rng);

    buf[off] ^= 1 << bit;
}

pub fn mut_interesting8(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.is_empty() {
        return;
    }

    let item_index = rng.gen_range(0..INTERESTING_8.len());
    let byte_index = choose_start_point(buf.len(), position, rng);
    buf[byte_index] = INTERESTING_8[item_index] as u8;
}

pub fn mut_interesting16(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    let len = buf.len();
    if len < 2 {
        return;
    }
    ...
    let pos = choose_start_point(len - 1, position, rng);
...
pub fn mut_interesting16_be(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    let len = buf.len();
    if len < 2 {
        return;
    }
...
pub fn mut_interesting32(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 4 {
        return;
    }
    ...
    let byte_index = choose_start_point(buf.len() - 3, position, rng);

    buf[byte_index..byte_index + 4].copy_from_slice(&INTERESTING_32[item_index].to_le_bytes());
}

pub fn mut_interesting32_be(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 4 {
        return;
    }
...
pub fn mut_arith8_sub(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.is_empty() {
        return;
    }
    let item: u8 = 1 + rng.gen_range(0..config::ARITH_MAX as u8);
    // let index = rng.gen_range(0..buf.len());
    let index = choose_start_point(buf.len(), position, rng);

    buf[index] = buf[index].wrapping_sub(item);
}

pub fn mut_arith8_add(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.is_empty() {
        return;
    }
    let item: u8 = 1 + rng.gen_range(0..config::ARITH_MAX as u8);
...
pub fn mut_arith16_sub(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }
    // let pos = rng.gen_range(0..buf.len() - 1);
    let pos = choose_start_point(buf.len() - 1, position, rng);
...
pub fn mut_arith16_sub_be(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }
    // let pos = rng.gen_range(0..buf.len() - 1);
...
pub fn mut_arith16_add(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }
    // let pos = rng.gen_range(0..buf.len() - 1);
...
pub fn mut_arith16_add_be(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }
    // let pos = rng.gen_range(0..buf.len() - 1);
...
pub fn mut_arith32_sub(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 4 {
        return;
    }
    // let pos = rng.gen_range(0..buf.len() - 3);
    let pos = choose_start_point(buf.len() - 3, position, rng);
...
pub fn mut_arith32_sub_be(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 4 {
        return;
    }
    // let pos = rng.gen_range(0..buf.len() - 3);
...
pub fn mut_arith32_add(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 4 {
        return;
    }
    // let pos = rng.gen_range(0..buf.len() - 3);
...
pub fn mut_arith32_add_be(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 4 {
        return;
    }
    // let pos = rng.gen_range(0..buf.len() - 3);
...
pub fn mut_rand8(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.is_empty() {
        return;
    }
    // let pos = rng.gen_range(0..buf.len());
    let pos = choose_start_point(buf.len(), position, rng);
    let item: u8 = 1 + rng.gen_range(0..255);
    buf[pos] ^= item;
}

pub fn mut_clone_copy(buf: &mut Vec<u8>, rng: &mut impl Rng, tmp_buf: &mut Vec<u8>, position: usize) {
    ...
    let clone_to = choose_start_point(buf.len(), position, rng) as u32;
...
pub fn mut_clone_fixed(buf: &mut Vec<u8>, rng: &mut impl Rng, tmp_buf: &mut Vec<u8>, position: usize) {
...
pub fn mut_overwrite_copy(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }
    ...
    loop {
        ...
        copy_to = choose_start_point(buf.len() - copy_len as usize + 1, position, rng) as u32;
...
pub fn mut_overwrite_fixed(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }
    ...
    let copy_to = choose_start_point(buf.len() - copy_len as usize + 1, position, rng);
    let strat = rng.gen_bool(0.5);
...
pub fn mut_byte_add(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.is_empty() {
        return;
    }
    // let idx = rng.gen_range(0..buf.len());
    let idx = choose_start_point(buf.len(), position, rng);
    buf[idx] = buf[idx].wrapping_add(1);
}

pub fn mut_byte_sub(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.is_empty() {
        return;
    }
    // let idx = rng.gen_range(0..buf.len());
...
pub fn mut_bitflip8(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.is_empty() {
        return;
    }
    // let idx = rng.gen_range(0..buf.len());
...
pub fn mut_switch(buf: &mut Vec<u8>, rng: &mut impl Rng, tmp_buf: &mut Vec<u8>, position: usize) {
    if buf.len() < 4 {
        return;
    }

    let mut switch_from = rng.gen_range(0..buf.len());
    let mut switch_to;
    
    loop {
        // switch_to = rng.gen_range(0..buf.len());
        switch_to = choose_start_point(buf.len(), position, rng);
        if switch_to != switch_from {
            break;
        }
        switch_from = rng.gen_range(0..buf.len());
    }
...
pub fn mut_del(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }
    ...
    let del_from = choose_start_point(buf.len() - del_len as usize + 1, position, rng);

    buf.drain(del_from..del_from + del_len as usize);
}

pub fn mut_shuffle(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 4 {
        return;
    }
    ...
    let off = choose_start_point(buf.len() - len as usize + 1, position, rng);
...
pub fn mut_del_one(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }

    // let del_from = rng.gen_range(0..buf.len());
    let del_from = choose_start_point(buf.len(), position, rng);
    buf.remove(del_from);
}

pub fn mut_insert_one(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    if buf.len() < 2 {
        return;
    }

    // let clone_to = rng.gen_range(0..=buf.len());
    let clone_to = choose_start_point(buf.len() + 1, position, rng);
    let strat = rng.gen_bool(0.5);
    let clone_from = if clone_to > 0 { clone_to - 1 } else { 0 };
...
pub fn mut_ascii_num(buf: &mut Vec<u8>, rng: &mut impl Rng, tmp_buf: &mut Vec<u8>, position: usize) {
    if buf.len() < 4 {
        return;
    }

    // let mut off = rng.gen_range(0..buf.len());
    let mut off = choose_start_point(buf.len(), position, rng);
    let mut off2 = off;
    let mut cnt = 0;
...
pub fn mut_insert_ascii_num(buf: &mut Vec<u8>, rng: &mut impl Rng, position: usize) {
    ...
    let pos = choose_start_point(buf.len() - len + 1, position, rng);
...
pub fn mut_splice_overwrite(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
    ...
    let copy_to = choose_start_point(buf.len() + 1 - copy_len as usize, position, rng);
...
pub fn mut_splice_insert(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, tmp_buf: &mut Vec<u8>, position: usize) {
    if (buf.len() + config::HAVOC_BLK_XL as usize) >= config::MAX_INPUT_LEN {
        return;
    }
    let splice_buf = handler.executor.random_splice_input_buf(handler.seed_info.seed_id);
...
// pub fn mut_trace_extra_insert(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
...
// pub fn mut_trace_extra_overwrite(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
//     if handler.strcmp_data_vec.len() == 0 {
//         return
//     }
//     let trace_extra = &handler.strcmp_data_vec[rng.gen_range(0..handler.strcmp_data_vec.len())];
...
//     let clone_to = choose_start_point(buf.len() - trace_extra.len + 1, position, rng);
//     buf.splice(clone_to..clone_to + trace_extra.len, trace_extra.data.iter().cloned());
// }

pub fn mut_trace_cmp_data(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
    ...
    let mut pos = choose_start_point(buf.len() - max_len + 1, position, rng);
...
pub fn mut_extra_insert(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
...
pub fn mut_extra_overwrite(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
    let extra_data = handler.get_extra_data();
    if extra_data.len() == 0 {
        return
    }
    ...
    debug!("trace_extra: {:?}", extra);

    let clone_to = choose_start_point(buf.len() + 1, position, rng);
    buf.splice(clone_to..clone_to + extra.len, extra.data.iter().cloned());
}

pub fn mut_const_strcmp_insert(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
...
pub fn mut_const_strcmp_overwrite(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
    if handler.const_strcmp_data_vec.len() == 0 {
        return
    }
    let trace_extra = &handler.const_strcmp_data_vec[rng.gen_range(0..handler.const_strcmp_data_vec.len())];
...
pub fn mut_const_cmp_data_be(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
    ...
    let byte_index = choose_start_point(buf.len() - value_len + 1, position, rng);

    buf[byte_index..byte_index + value_len].copy_from_slice(&bytes);
}

pub fn mut_const_cmp_data(buf: &mut Vec<u8>, handler: &mut SearchHandler, rng: &mut impl Rng, position: usize) {
    if handler.const_cmp_data_vec.len() == 0 {
        return;
    }
    let trace_cmp_data = handler.const_cmp_data_vec[rng.gen_range(0..handler.const_cmp_data_vec.len())];
