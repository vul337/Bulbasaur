...
bool CmpLogRoutines::hookRtns(Module &M) {
      ...
      for (auto &IN : BB) {
        ...
        if ((callInst = dyn_cast<CallInst>(&IN))) {
          ...
          */
          ...
          if (isGccStdStringCString || isGccStdStringStdString ||
            isLlvmStdStringStdString || isLlvmStdStringCString || isMemcmp ||
            isStrcmp || isStrncmp || isPtrRtn || isPtrRtnN) {
              function_cnt++;
          }
  ...
  llvm::errs() << "calls: " << calls.size() << "\n";
  llvm::errs() << "gccStdStd: " << gccStdStd.size() << "\n";
  llvm::errs() << "gccStdC: " << gccStdC.size() << "\n";
  llvm::errs() << "llvmStdStd: " << llvmStdStd.size() << "\n";
  llvm::errs() << "llvmStdC: " << llvmStdC.size() << "\n";
  llvm::errs() << "Memcmp: " << Memcmp.size() << "\n";
  llvm::errs() << "Strcmp: " << Strcmp.size() << "\n";
  llvm::errs() << "Strncmp: " << Strncmp.size() << "\n";
