...
//===----------------------------------------------------------------------===//
//
// Bulbasaur: bulbasaur-cov-debug.so.cc  --  Debug / Source-Location Pass
// ======================================================================
//
// A static analysis pass that records the source-code location of every
// instrumented branch and writes the results to CSV files at compile time.
// It does NOT need to run during the actual fuzzing session — its output
// is consumed offline by analysis tools.
//
// What it emits (set BULBASAUR_BRANCH_LOC_PATH=<dir> before building the target):
//   <dir>/branch_loc.csv   – one row per branch:
//       branch_id, source_file, line, column, branch_type, function_name
//   <dir>/function_loc.csv – one row per instrumented function:
//       function_name, source_file, line
//
// Role in the pipeline:
//   - Compile the target once with USE_DEBUG=1 / BULBASAUR_INST_MODE=DEBUG to
//     generate the CSV tables.
//   - Load the CSVs into the Bulbasaur analyser to map branch IDs (as
//     reported by the Full/Fast passes at runtime) back to source locations.
//   - The pass still instruments the binary normally (NoPrune=true) so the
//     branch_ids in the CSV match those produced by the Full pass at runtime.
//
// Env vars (at compile / link time):
//   BULBASAUR_BRANCH_LOC_PATH – directory where CSV files are written.
//                           If unset, location output is silently skipped.
//   AFL_DEBUG, AFL_QUIET  – verbosity controls (same as other passes).
//
// Built as: bulbasaur-cov-debug.so   (loaded via -fpass-plugin=)
// Plugin ID: "BulbasaurCovDebug"
//
//===----------------------------------------------------------------------===//
...
const char trace_br8[] = "trace_br8";
const char trace_br16[] = "trace_br16";
const char trace_br32[] = "trace_br32";
const char trace_br64[] = "trace_br64";

const char strcmp_trace[] = "strcmp_trace";
const char strncmp_trace[] = "strncmp_trace";
const char memcmp_trace[] = "memcmp_trace";
const char strstr_trace[] = "strstr_trace";
const char ptr_rtn_trace[] = "ptr_rtn_trace";
const char ptr_rtn_gcc_stdstring_stdstring_trace[] = "ptr_rtn_gcc_stdstring_stdstring_trace";
const char ptr_rtn_gcc_stdstring_cstring_trace[] = "ptr_rtn_gcc_stdstring_cstring_trace";
const char ptr_rtn_llvm_stdstring_stdstring_trace[] = "ptr_rtn_llvm_stdstring_stdstring_trace";
const char ptr_rtn_llvm_stdstring_cstring_trace[] = "ptr_rtn_llvm_stdstring_cstring_trace";
...
const char DebugInfoSectionName[] = "debug_info";
const char EdgeToPredBranchSectionName[] = "edge_to_pred_branch";
const char EdgeDebugInfoSectionName[] = "edge_debug_info";
...
class BulbasaurCoverageDebugPass
    : public PassInfoMixin<BulbasaurCoverageDebugPass> {

 public:
  BulbasaurCoverageDebugPass(
 ...
 private:
  ...
  GlobalVariable *CmpTracePtr = NULL;
  ConstantInt    *One = NULL;
  ConstantInt    *Zero = NULL;

  std::string branch_loc_path = "";
  std::string function_loc_path = "";
...
llvmGetPassPluginInfo() {
  ...
  return {LLVM_PLUGIN_API_VERSION, "BulbasaurCovDebug", "v1.0",
          ...
          [](PassBuilder &PB) {
            ...
            PB.registerOptimizerLastEPCallback(
                ...
                [](ModulePassManager &MPM, OptimizationLevel OL) {
                  ...
                  MPM.addPass(BulbasaurCoverageDebugPass());
...
PreservedAnalyses BulbasaurCoverageDebugPass::run(Module                &M,
                                                  ModuleAnalysisManager &MAM) {

  BulbasaurCoverageDebugPass ModuleSancov(Options);
...
std::pair<Value *, Value *> BulbasaurCoverageDebugPass::CreateSecStartEnd(
...
Function *BulbasaurCoverageDebugPass::CreateInitCallsForSections(
    Module &M, const char *CtorName, const char *InitFunctionName, Type *Ty,
    const char *Section, const char *BranchSection, const char *BranchTypeSectionName,
    const char *EdgeToPredBranchSectionName) {
...
bool BulbasaurCoverageDebugPass::instrumentModule(
  ...
  if ((isatty(2) && !getenv("AFL_QUIET")) || debug) {
    ...
    SAYF(cCYA "BulbasaurCov[Debug]" VERSION cRST "\n");
  ...
  const char *branch_loc_env = getenv("BULBASAUR_BRANCH_LOC_PATH");
  if (branch_loc_env) {
    std::string dir_path = std::string(branch_loc_env);
    
    // Verify directory exists and is accessible
    struct stat path_stat;
    if (stat(dir_path.c_str(), &path_stat) != 0) {
    ...
    // Check if directory is accessible (try to open)
    ...
    // Set file paths
    branch_loc_path = dir_path + "/branch_loc.csv";
    function_loc_path = dir_path + "/function_loc.csv";
  }
  ...
  CmpTracePtr = M.getNamedGlobal("__cmp_trace_ptr");

  if (!CmpTracePtr) {

    CmpTracePtr = new GlobalVariable(M, PointerType::get(Int8Ty, 0), false,
  ...
  BranchTraceCmpFunction[0] =
      M.getOrInsertFunction(trace_br8, BranchTraceCmpZeroExtAL, VoidTy, Int32Ty,
                         Int8Ty, Int8Ty);
  BranchTraceCmpFunction[1] =
      M.getOrInsertFunction(trace_br16, BranchTraceCmpZeroExtAL, VoidTy, Int32Ty,
                         Int16Ty, Int16Ty);
  BranchTraceCmpFunction[2] =
      M.getOrInsertFunction(trace_br32, BranchTraceCmpZeroExtAL, VoidTy, Int32Ty,
                         Int32Ty, Int32Ty);
  BranchTraceCmpFunction[3] =
      M.getOrInsertFunction(trace_br64, BranchTraceCmpZeroExtAL, VoidTy, Int32Ty, Int64Ty, Int64Ty);
  
  BranchTraceStrcmpFunction[0] =
      M.getOrInsertFunction(strcmp_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  BranchTraceStrcmpFunction[1] =
      M.getOrInsertFunction(strncmp_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  BranchTraceStrcmpFunction[2] =
      M.getOrInsertFunction(memcmp_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  BranchTraceStrcmpFunction[3] =
      M.getOrInsertFunction(strstr_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  BranchTraceStrcmpFunction[4] =
      M.getOrInsertFunction(ptr_rtn_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  BranchTraceStrcmpFunction[5] =
      M.getOrInsertFunction(ptr_rtn_gcc_stdstring_stdstring_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  BranchTraceStrcmpFunction[6] =
      M.getOrInsertFunction(ptr_rtn_gcc_stdstring_cstring_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  BranchTraceStrcmpFunction[7] =
      M.getOrInsertFunction(ptr_rtn_llvm_stdstring_stdstring_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  BranchTraceStrcmpFunction[8] =
      M.getOrInsertFunction(ptr_rtn_llvm_stdstring_cstring_trace, VoidTy,Int32Ty,
                         Int8PtrTy, Int8PtrTy, Int64Ty);
  ...
  if (!be_quiet) {
    ...
    if (!instr) {
      ...
      OKF("[SanitizerCoveragePCGUARDDebug] Instrumented %u locations with no collisions (%s mode) of which are "
          "%u handled and %u unhandled selects.",
          instr, modeline, selects, unhandled);
      OKF("[SanitizerCoveragePCGUARDDebug] Instrumented %u valid branches, calls cnt %u, gccstdstd cnt %u, "
...
bool BulbasaurCoverageDebugPass::IsInterestingFunctionCall(CallInst *callInst, u8 favor_std, u8 favor_ptr_rtn) {
...
void BulbasaurCoverageDebugPass::instrumentFunction(
  ...
  if (function_loc_path != "") {
    static std::ofstream function_loc_file;
    static bool opened = false;
    static std::unordered_set<std::string> seen;
  
    if (!opened) {
      function_loc_file.open(function_loc_path, std::ios::app);
      opened = true;
      function_loc_file << "function_name,function_location\n";
    }
    ...
    if (!SP->getLinkageName().empty()) {
      function_name = SP->getLinkageName().str();
    } else {
    ...
    if (!file && SP->getUnit())
      file = SP->getUnit()->getFile();
  
    std::string loc;
    if (file && SP->getScopeLine() > 0) {
      std::string dir = file->getDirectory().str();
      std::string name = file->getFilename().str();
      loc = (dir.empty() ? name : dir + "/" + name)
            + ":" + std::to_string(SP->getScopeLine()) + ":0";
    }
  
    function_loc_file << function_name << "," << loc << "\n";
  }
  ...
  if (debug) {
    ...
    fprintf(stderr, "BulbasaurCov[Debug]: instrumenting %s in %s\n",
...
//   // Record the BBs instrumented by full mode.
...
//     fprintf(stderr, "SanitizerCoveragePCGUARDDebug: instrumenting %s in %s\n",
//             F.getName().str().c_str(), F.getParent()->getName().str().c_str());
...
GlobalVariable *BulbasaurCoverageDebugPass::CreateFunctionLocalArrayInSection(
...
GlobalVariable *BulbasaurCoverageDebugPass::CreatePCArray(
...
void BulbasaurCoverageDebugPass::CreateFunctionLocalArrays(
...
bool BulbasaurCoverageDebugPass::InjectCoverage(
  ...
  for (auto &BB : F) {
    ...
    for (auto &IN : BB) {
      ...
      if ((callInst = dyn_cast<CallInst>(&IN))) {
        ...
        BulbasaurCoverageDebugPass::SetNoSanitizeMetadata(Idx);
      ...
      if (!skip_next && (selectInst = dyn_cast<SelectInst>(&IN))) {
        ...
        BulbasaurCoverageDebugPass::SetNoSanitizeMetadata(MapPtr);
        ...
        while (1) {
          ...
          if (!vector_cnt) {
            ...
            BulbasaurCoverageDebugPass::SetNoSanitizeMetadata(CurLoc);
            ...
            BulbasaurCoverageDebugPass::SetNoSanitizeMetadata(elementld);
          ...
          if (use_threadsafe_counters) {
            ...
            BulbasaurCoverageDebugPass::SetNoSanitizeMetadata(Counter);
            ...
            BulbasaurCoverageDebugPass::SetNoSanitizeMetadata(StoreCtx);
...
void BulbasaurCoverageDebugPass::InjectTraceForSwitch(
...
void BulbasaurCoverageDebugPass::InjectTraceForCmp(
...
void BulbasaurCoverageDebugPass::InjectCoverageAtBlock(Function   &F,
                                                       BasicBlock &BB,
                                                       size_t      Idx,
                                                       DenseMap<Instruction *, size_t> &SancovMapIndex, 
                                                       bool        IsLeafFunc) {
...
std::string BulbasaurCoverageDebugPass::getSectionName(
...
std::string BulbasaurCoverageDebugPass::getSectionStart(
    const std::string &Section) const {
...
std::string BulbasaurCoverageDebugPass::getSectionEnd(
    const std::string &Section) const {
...
bool BulbasaurCoverageDebugPass::isSigned(Value *V0, Value *V1) {
...
void BulbasaurCoverageDebugPass::TraceCmpBranch(
  ...
  // Open file once before the loop for better performance
  std::ofstream branch_loc_file;
  bool need_csv_header = false;
  if (branch_loc_path != "") {
    // Check if file exists and is empty to determine if we need CSV header
    std::ifstream check_file(branch_loc_path);
    need_csv_header = check_file.peek() == std::ifstream::traits_type::eof();
    check_file.close();
    
    branch_loc_file.open(branch_loc_path, std::ios::app);
    if (!branch_loc_file.is_open()) {
      errs() << "Error: Failed to open branch location file: " << branch_loc_path << "\n";
    } else if (need_csv_header) {
      // Write CSV header if file is empty
      branch_loc_file << "hash_val,location_str,function_name\n";
    }
  }
  ...
  for (auto I : CmpTraceTargets) {
    ...
    if (CmpInst *CMP = dyn_cast<CmpInst>(I)) {
      ...
      // Record debug information of branch
      if (branch_loc_path != "" && branch_loc_file.is_open()) {
        DILocation *loc = CMP->getDebugLoc();
        uint64_t hash_val = 0;
        if (loc) {
          ...
          std::string location_str = file_path + ":" + 
                                    std::to_string(loc->getLine()) + ":" + 
                                    std::to_string(loc->getColumn());

          hash_val = hash_value(StringRef(location_str));
          ...
          errs() << "Warning: CMP instruction has no debug location\n";
          hash_val = 0;
        }
    
        Constant *DebugInfo = ConstantInt::get(Int64Ty, hash_val);

        Constant* TypeStructInit = ConstantStruct::get(MyStructType, {BranchArrayElemPtrInt, DebugInfo});
    
        GlobalVariable* DebugInfoStructGlobal = new GlobalVariable(*CurModule, MyStructType, true, GlobalValue::InternalLinkage, TypeStructInit, "debug_info");
        if (TargetTriple.supportsCOMDAT() && (TargetTriple.isOSBinFormatELF() || !F.isInterposable()))
          if (auto Comdat = getOrCreateFunctionComdat(F, TargetTriple))
            DebugInfoStructGlobal->setComdat(Comdat);
    
        DebugInfoStructGlobal->setSection(std::string("__") + DebugInfoSectionName);
        if (DebugInfoStructGlobal->hasComdat())
          GlobalsToAppendToCompilerUsed.push_back(DebugInfoStructGlobal);
        else
          GlobalsToAppendToUsed.push_back(DebugInfoStructGlobal);
      }

      // Record debug information of succ branch.
      if (branch_loc_path != "" && branch_loc_file.is_open()) {
        for (auto succ_sancov: SancovForCmpSuccBB[iter_cnt]) {
          ...
          BasicBlock *bb = succ_sancov->getParent();
          ...
          Constant *EdgeDebugInfo = ConstantInt::get(Int64Ty, hash_val);
          Constant* EdgeDebugInfoStructInit = ConstantStruct::get(MyStructType, {ArrayElemPtrInt, EdgeDebugInfo});
      
          GlobalVariable* EdgeDebugInfoStructGlobal = new GlobalVariable(*CurModule, MyStructType, true, GlobalValue::InternalLinkage, EdgeDebugInfoStructInit, "edge_debug_info");
          if (TargetTriple.supportsCOMDAT() && (TargetTriple.isOSBinFormatELF() || !F.isInterposable()))
            if (auto Comdat = getOrCreateFunctionComdat(F, TargetTriple))
              EdgeDebugInfoStructGlobal->setComdat(Comdat);
      
          EdgeDebugInfoStructGlobal->setSection(std::string("__") + EdgeDebugInfoSectionName);
          if (EdgeDebugInfoStructGlobal->hasComdat())
            GlobalsToAppendToCompilerUsed.push_back(EdgeDebugInfoStructGlobal);
          else
            GlobalsToAppendToUsed.push_back(EdgeDebugInfoStructGlobal);
  ...
  if (branch_loc_file.is_open()) {
    branch_loc_file.close();
  }
}


void BulbasaurCoverageDebugPass::TraceStrcmpBranch(
...
void BulbasaurCoverageDebugPass::TraceNonTermStrcmp(
  ...
  for (auto I : StrcmpTraceTargetsNonTerminator) {
    ...
    if (CallInst *callInst = dyn_cast<CallInst>(I)) {
      ...
      if (branch_loc_path != "" && branch_loc_file.is_open()) {
        ...
        // Default debug information for non-terminator branches.
        Constant *DebugInfo = ConstantInt::get(Int64Ty, 0);
        TypeStructInit = ConstantStruct::get(MyStructType, {BranchArrayElemPtrInt, DebugInfo});
...
void BulbasaurCoverageDebugPass::TraceSwitchBranch(Function &F, ArrayRef<Instruction *> SwitchTraceTargets, ArrayRef<Instruction *> SancovForSwitch, ArrayRef<Instruction *> case_target_list, ArrayRef<ConstantInt *> case_val_list, std::vector<int> int_val_list, DenseMap<Instruction *, size_t> &SancovMapIndex) {
  ...
  // As with comparison branches, avoid emitting an empty branch metadata
  // array when this function has no switch cases. Some LLVM 15 backends
  // reject zero-sized globals in custom sections during code generation.
  if (valid_cmp_br_cnt == 0) {
    return;
  }
    ...
    if (SwitchInst *SI = dyn_cast<SwitchInst>(I)) {
      ...
      for (auto &Case : SI->cases()) {
        ...
        if (branch_loc_path != "" && branch_loc_file.is_open()) {
          ...
          // Get the first debug location in the basic block
          for (Instruction &I : *BB) {
            if (loc = I.getDebugLoc()) {
                break;
            }
          ...
          Instruction *succ_sancov = case_target_list[debugCaseCnt];
          size_t succ_index = SancovMapIndex[succ_sancov];
          Constant *ArrayElemPtr = ConstantExpr::getGetElementPtr(FunctionGuardArray->getValueType(), FunctionGuardArray, ArrayRef<Constant*>({Builder.getInt32(0), Builder.getInt32(succ_index)}));
          Constant *ArrayElemPtrInt = ConstantExpr::getPtrToInt(ArrayElemPtr, Int64Ty);
          ...
          TypeStructInit = ConstantStruct::get(MyStructType, {ArrayElemPtrInt, DebugInfo});
          DebugInfoStructGlobal = new GlobalVariable(*CurModule, MyStructType, true, GlobalValue::InternalLinkage, TypeStructInit, "edge_debug_info");
          if (TargetTriple.supportsCOMDAT() && (TargetTriple.isOSBinFormatELF() || !F.isInterposable()))
            if (auto Comdat = getOrCreateFunctionComdat(F, TargetTriple))
              DebugInfoStructGlobal->setComdat(Comdat);
          
          DebugInfoStructGlobal->setSection(std::string("__") + EdgeDebugInfoSectionName);
          if (DebugInfoStructGlobal->hasComdat())
            GlobalsToAppendToCompilerUsed.push_back(DebugInfoStructGlobal);
          else
            GlobalsToAppendToUsed.push_back(DebugInfoStructGlobal);
      ...
      if (branch_loc_path != "" && branch_loc_file.is_open()) {
        ...
        // Default debug information for default case.
        Constant *DebugInfo = ConstantInt::get(Int64Ty, 0);
        TypeStructInit = ConstantStruct::get(MyStructType, {BranchArrayElemPtrInt, DebugInfo});
