...
u32 forksrv_fd = -1;
u16 *__frontier_branch_map_ptr; // max prefix match during execution.
u32 __branch_map_loc = 0; // size of branch map.

/* Bulbasaur Trace Addition. */
#define FRONTIER_CMP_TRACE_VAR "FRONTIER_CMP_TRACE_SHM_ID"  // __cmp_trace_ptr

#define UNIT_MAX_TRACE_CNT 32
...
struct cmp_trace {
  u16 size;
  struct cmp_unit units[UNIT_MAX_TRACE_CNT];
} __attribute__((packed));
...
struct strcmp_trace {
  u16 size;
  struct strcmp_unit units[UNIT_MAX_TRACE_CNT];
} __attribute__((packed));

struct cmp_trace *__cmp_trace_ptr = NULL;

struct branch_type_unit {
  u64 branch_id;
  u64 branch_type;
} __attribute__((packed));

struct branch_type_unit *__branch_type_dict = NULL;
u32 __branch_type_loc = 0;

struct edge_to_pred_branch_unit {
  u64 edge_id;
  u64 branch_id;
} __attribute__((packed));

struct edge_to_pred_branch_unit *__edge_to_pred_branch_dict = NULL;
u32 __edge_to_pred_branch_loc = 0;
...
static void __afl_map_shm(void) {
  ...
  id_str = getenv(FRONTIER_BRANCH_SHM_VAR);
  if (id_str) {

    u32 shm_id = atoi(id_str);
    __frontier_branch_map_ptr = (u16 *)shmat(shm_id, NULL, 0);

    if (!__frontier_branch_map_ptr) {
      perror("shmat for __frontier_branch_map_ptr");
      send_forkserver_error(FS_ERROR_SHM_OPEN);
      _exit(1);
    }

  } else {
    __frontier_branch_map_ptr = (u16 *) __afl_area_initial;
    // When not in fast target, we don't initialize __frontier_branch_map_ptr. It's fine...
    // perror("FRONTIER_BRANCH_SHM_VAR not found.");
    // send_forkserver_error(FS_ERROR_SHM_OPEN);
    // _exit(1);
  }

  id_str = getenv(FRONTIER_CMP_TRACE_VAR);
  if (id_str) {

    u32 shm_id = atoi(id_str);
    __cmp_trace_ptr = (struct cmp_trace *)shmat(shm_id, NULL, 0);

    if (!__cmp_trace_ptr) {
      perror("shmat for __cmp_trace_ptr");
      send_forkserver_error(FS_ERROR_SHM_OPEN);
      _exit(1);
    }

  } else {
    // __cmp_trace_ptr = (struct cmp_trace *) __afl_area_initial;
    // When not in trace target, we don't initialize __cmp_trace_ptr. It's fine...
    // perror("FRONTIER_BRANCH_SHM_VAR not found.");
    // send_forkserver_error(FS_ERROR_SHM_OPEN);
    // _exit(1);
  }
...
static void __afl_unmap_shm(void) {
  ...
  id_str = getenv(FRONTIER_BRANCH_SHM_VAR);
  if (id_str) {
    shmdt((void *)__frontier_branch_map_ptr);
    __frontier_branch_map_ptr = NULL;
  }
...
static void __afl_start_forkserver(void) {
  ...
  /*
   * LLVM's native PCGuard instrumentation calls the two-argument
   * __sanitizer_cov_trace_pc_guard_init callback, so it does not provide the
   * optional Bulbasaur branch-map range.  The forkserver protocol still
   * carries a branch-map size, and the fuzzer allocates that shared memory
   * unconditionally.  Advertise a single sentinel slot when no richer branch
   * instrumentation populated the counter; this keeps native fallback builds
   * usable while preserving the existing branch ids for custom builds.
   */
  if (__branch_map_loc < 1) __branch_map_loc = 1;
  ...
  if (!__afl_old_forkserver) {
    ...
    if (getenv("READ_ELF_SECTION_DATA") != NULL) {

      // send branch_type section data.
      u32 len = __branch_type_loc * sizeof(struct branch_type_unit), offset = 0;

      if (write(forksrv_fd + 1, &len, 4) != 4) { _exit(1); }

      while (len != 0) {

        s32 ret;
        ret = write(forksrv_fd + 1, ((u8 *) __branch_type_dict) + offset, len);
      ...
      // send edge_to_pred_branch section data.
      len = __edge_to_pred_branch_loc * sizeof(struct edge_to_pred_branch_unit), offset = 0;
      ...
      while (len != 0) {
        ...
        s32 ret;
        ret = write(forksrv_fd + 1, ((u8 *) __edge_to_pred_branch_dict) + offset, len);
...
__attribute__((constructor(0))) void __afl_auto_first(void) {
  ...
  char *forksrv_fd_str = getenv(PARAFUZZ_FORKSRV_FD_VAR);
  if (forksrv_fd_str) {
    forksrv_fd = atoi(forksrv_fd_str);
    if (forksrv_fd <= 0) {
        fprintf(stderr, "Error: Invalid forksrv_fd: %s\n", forksrv_fd_str);
        _exit(1);
    }
  } else {
    forksrv_fd = 111;
    // fprintf(stderr, "Error: PARAFUZZ_FORKSRV_FD_VAR not set\n");
    // _exit(1);
  }
...
void __sanitizer_cov_trace_pc_guard_init(uint32_t *start, uint32_t *stop,
                                          uint32_t *branch_start, uint32_t *branch_stop, 
                                          uint64_t *branch_type_start, uint64_t *branch_type_stop,
                                          uint64_t *edge_to_pred_branch_start, uint64_t *edge_to_pred_branch_stop) {
  ...
  *(start++) = ++__afl_final_loc;
  ...
  // Skip branch[0]. It is tag byte.
  if (branch_start == branch_stop || *branch_start) { return; }

  if (__branch_map_loc < 1) __branch_map_loc = 1;

  *(branch_start++) = __branch_map_loc++;

  while (branch_start < branch_stop) {
    *(branch_start++) = __branch_map_loc++;
  }

  // If READ_ELF_SECTION_DATA detected, we will parse sections and send data 
  if (getenv("READ_ELF_SECTION_DATA") != NULL) {

    if (branch_type_start != NULL && branch_type_stop != NULL && branch_type_start != branch_type_stop) {
      __branch_type_dict = (struct branch_type_unit *) realloc(__branch_type_dict, 
                                                      sizeof(struct branch_type_unit) * __branch_map_loc);

      while (branch_type_start < branch_type_stop) {
        __branch_type_dict[__branch_type_loc].branch_id = *(uint32_t *)(*branch_type_start);
        __branch_type_dict[__branch_type_loc].branch_type = *(branch_type_start + 1);
        branch_type_start += 2;
        __branch_type_loc ++;
      }
    }
    
    if (edge_to_pred_branch_start != NULL && edge_to_pred_branch_stop != NULL && edge_to_pred_branch_start != edge_to_pred_branch_stop) {
      __edge_to_pred_branch_dict = (struct edge_to_pred_branch_unit *) realloc(__edge_to_pred_branch_dict,
                                                      sizeof(struct edge_to_pred_branch_unit) * __afl_final_loc);

      while (edge_to_pred_branch_start < edge_to_pred_branch_stop) {
        __edge_to_pred_branch_dict[__edge_to_pred_branch_loc].edge_id = *(uint32_t *)(*edge_to_pred_branch_start);
        __edge_to_pred_branch_dict[__edge_to_pred_branch_loc].branch_id = *(uint32_t *)(*(edge_to_pred_branch_start + 1));
        edge_to_pred_branch_start += 2;
        __edge_to_pred_branch_loc ++;
      }
    }
  }
...
void log_br8(int addr, s8 op1, s8 op2) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  s8 diff = op1 ^ op2;
  u16 prefix_bits = __builtin_clz((u32)diff << 24);
  if (prefix_bits == 32) prefix_bits = 8;
  u16 tmp_cnt = (((__frontier_branch_map_ptr[addr] >> 10) + 1) & 0x3F) << 10;
  if (tmp_cnt == 0) tmp_cnt = (1 << 10);

  prefix_bits++;
  if ((__frontier_branch_map_ptr[addr] & 0x3FF) < prefix_bits) {
    __frontier_branch_map_ptr[addr] = prefix_bits | tmp_cnt;
  } else {
    __frontier_branch_map_ptr[addr] = (__frontier_branch_map_ptr[addr] & 0x3FF) | tmp_cnt;
  }
  return;
}
void log_br16(int addr, s16 op1, s16 op2) {
  ...
  s16 diff = op1 ^ op2;
  u16 prefix_bits = __builtin_clz((u32)diff << 16);
  if (prefix_bits == 32) prefix_bits = 16;
  u16 tmp_cnt = (((__frontier_branch_map_ptr[addr] >> 10) + 1) & 0x3F) << 10;
  if (tmp_cnt == 0) tmp_cnt = (1 << 10);
...
void log_br32(int addr, s32 op1, s32 op2) {
  ...
  s32 diff = op1 ^ op2;
  u16 prefix_bits = __builtin_clz((u32)diff);
  u16 tmp_cnt = (((__frontier_branch_map_ptr[addr] >> 10) + 1) & 0x3F) << 10;
  if (tmp_cnt == 0) tmp_cnt = (1 << 10);
...
void log_br64(int addr, s64 op1, s64 op2) {
  ...
  s64 diff = op1 ^ op2;
  u16 prefix_bits = __builtin_clzll((u64)diff);
  u16 tmp_cnt = (((__frontier_branch_map_ptr[addr] >> 10) + 1) & 0x3F) << 10;
  if (tmp_cnt == 0) tmp_cnt = (1 << 10);
...
// addr: branch ID, initialized during ELF loading.
// op1: constant or variable string.
// op2: variable string.
// str_len: maximum comparison length for strcmp/strstr; actual comparison length for strncmp/memcmp.
void strcmp_log(int addr, u8* op1, u8* op2, unsigned long str_len) {
  ...
  str_len = min(STRCMP_MAX_LEN, str_len);
  ...
  for (int i = 0; i < str_len; i++) {
    ...
    u8 diff = op1[i] ^ op2[i];
    prefix_bits += __builtin_clz((u32)diff << 24);
    if (terminate_flag) break;
  }
  u16 tmp_cnt = (((__frontier_branch_map_ptr[addr] >> 10) + 1) & 0x3F) << 10;
  if (tmp_cnt == 0) tmp_cnt = (1 << 10);
...
void strncmp_log(int addr, u8* op1, u8* op2, unsigned long str_len) {
  ...
  if (op1 == NULL || op2 == NULL) return;
  str_len = min(str_len, STRCMP_MAX_LEN);
...
void ptr_rtn_log(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  if (op1 == NULL || op2 == NULL) return;
  int len1, len2;
  if ((len1 = area_is_valid(op1, STRCMP_MAX_LEN)) <= 0 ||
        (len2 = area_is_valid(op2, STRCMP_MAX_LEN)) <= 0) return;
...
void ptr_rtn_gcc_stdstring_stdstring_log(int addr, u8 *stdstring1, u8 *stdstring2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  if (stdstring1 == NULL || stdstring2 == NULL) return;
  str_len = min(STRCMP_MAX_LEN, str_len);
  if (area_is_valid(stdstring1, str_len) <= 0 || area_is_valid(stdstring2, str_len) <= 0)
    return;

  ptr_rtn_log(addr, get_gcc_stdstring(stdstring1),
                    get_gcc_stdstring(stdstring2), str_len);
}

void ptr_rtn_gcc_stdstring_cstring_log(int addr, u8 *stdstring, u8 *cstring, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  if (stdstring == NULL || cstring == NULL) return;
  str_len = min(STRCMP_MAX_LEN, str_len);
  if (area_is_valid(stdstring, str_len) <= 0 || area_is_valid(cstring, str_len) <= 0)
    return;

  ptr_rtn_log(addr, cstring, get_gcc_stdstring(stdstring), str_len);

}

void ptr_rtn_llvm_stdstring_cstring_log(int addr, u8 *stdstring, u8 *cstring, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  ptr_rtn_log(addr, cstring, get_llvm_stdstring(stdstring), str_len);
}

void ptr_rtn_llvm_stdstring_stdstring_log(int addr, u8 *stdstring1, u8 *stdstring2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  ptr_rtn_log(addr, get_llvm_stdstring(stdstring1), get_llvm_stdstring(stdstring2), str_len);
}

void non_term_strcmp_log(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  prefix_bits++;
  // if op1 equals op2 exactly, set prefix_bits to max value (0x3FF).
  if (is_equal) prefix_bits = 0x3FF;
  if ((__frontier_branch_map_ptr[addr] & 0x3FF) < prefix_bits) {
    __frontier_branch_map_ptr[addr] = prefix_bits | tmp_cnt;
  } else {
    __frontier_branch_map_ptr[addr] = (__frontier_branch_map_ptr[addr] & 0x3FF) | tmp_cnt;
...
void non_term_strncmp_log(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
...
void non_term_strstr_log(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
...
void non_term_memcmp_log(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
...
void non_term_ptr_rtn_log(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
...
void non_term_ptr_rtn_gcc_stdstring_stdstring_log(int addr, u8 *stdstring1, u8 *stdstring2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  non_term_ptr_rtn_log(addr, get_gcc_stdstring(stdstring1),
                    get_gcc_stdstring(stdstring2), str_len);
}

void non_term_ptr_rtn_gcc_stdstring_cstring_log(int addr, u8 *stdstring, u8 *cstring, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  non_term_ptr_rtn_log(addr, cstring, get_gcc_stdstring(stdstring), str_len);

}

void non_term_ptr_rtn_llvm_stdstring_cstring_log(int addr, u8 *stdstring, u8 *cstring, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  non_term_ptr_rtn_log(addr, cstring, get_llvm_stdstring(stdstring), str_len);
}

void non_term_ptr_rtn_llvm_stdstring_stdstring_log(int addr, u8 *stdstring1, u8 *stdstring2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  non_term_ptr_rtn_log(addr, get_llvm_stdstring(stdstring1), get_llvm_stdstring(stdstring2), str_len);
}


void trace_br8(int addr, s8 op1, s8 op2) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
  ...
  u16 size_index = __cmp_trace_ptr[addr].size % UNIT_MAX_TRACE_CNT;

  if (__cmp_trace_ptr[addr].units[size_index].match < prefix_bits) {
    __cmp_trace_ptr[addr].units[size_index].len = 1;
    __cmp_trace_ptr[addr].units[size_index].match = prefix_bits;
    __cmp_trace_ptr[addr].units[size_index].op1 = (u8) op1;
    __cmp_trace_ptr[addr].units[size_index].op2 = (u8) op2;
  }

  __cmp_trace_ptr[addr].size = min(__cmp_trace_ptr[addr].size + 1, STRCMP_MAX_CNT);

  return;
}

void trace_br16(int addr, s16 op1, s16 op2) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
  ...
  if (__cmp_trace_ptr[addr].units[size_index].match < prefix_bits) {
    __cmp_trace_ptr[addr].units[size_index].len = 2;
    __cmp_trace_ptr[addr].units[size_index].match = prefix_bits;
    __cmp_trace_ptr[addr].units[size_index].op1 = (u16) op1;
    __cmp_trace_ptr[addr].units[size_index].op2 = (u16) op2;
  }
...
void trace_br32(int addr, s32 op1, s32 op2) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
  ...
  if (__cmp_trace_ptr[addr].units[size_index].match < prefix_bits) {
    __cmp_trace_ptr[addr].units[size_index].len = 4;
    __cmp_trace_ptr[addr].units[size_index].match = prefix_bits;
    __cmp_trace_ptr[addr].units[size_index].op1 = (u32) op1;
    __cmp_trace_ptr[addr].units[size_index].op2 = (u32) op2;
  }
...
void trace_br64(int addr, s64 op1, s64 op2) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
  ...
  if (__cmp_trace_ptr[addr].units[size_index].match < prefix_bits) {
    __cmp_trace_ptr[addr].units[size_index].len = 8;
    __cmp_trace_ptr[addr].units[size_index].match = prefix_bits;
    __cmp_trace_ptr[addr].units[size_index].op1 = (u64) op1;
    __cmp_trace_ptr[addr].units[size_index].op2 = (u64) op2;
  }
...
void trace_br8_unsign(int addr, u8 op1, u8 op2) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
...
void trace_br16_unsign(int addr, u16 op1, u16 op2) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
...
void trace_br32_unsign(int addr, u32 op1, u32 op2) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
...
void trace_br64_unsign(int addr, u64 op1, u64 op2) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
...
// op2: variable string.
// str_len: maximum comparison length for strcmp/strstr; actual comparison length for strncmp/memcmp.
void strcmp_trace(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
  ...
  if (op1 == NULL || op2 == NULL) return;
  str_len = min(str_len, STRCMP_TRACE_MAX_LEN);
  ...
  int len1 = min(safe_strlen(op1, STRCMP_TRACE_MAX_LEN), STRCMP_TRACE_MAX_LEN);
  int len2 = min(safe_strlen(op2, STRCMP_TRACE_MAX_LEN), STRCMP_TRACE_MAX_LEN);

  struct strcmp_trace *__strcmp_trace_ptr = (struct strcmp_trace *) &__cmp_trace_ptr[addr];

  u16 size_index = __strcmp_trace_ptr->size % UNIT_MAX_TRACE_CNT;

  if (__strcmp_trace_ptr->units[size_index].match < prefix_bits) {
    __strcmp_trace_ptr->units[size_index].len1 = len1;
    __strcmp_trace_ptr->units[size_index].len2 = len2;
    __strcmp_trace_ptr->units[size_index].len3 = STRCMP_TRACE_MAX_LEN;
    __strcmp_trace_ptr->units[size_index].match = prefix_bits;
    __builtin_memcpy(__strcmp_trace_ptr->units[size_index].op1, op1, len1);
    __builtin_memcpy(__strcmp_trace_ptr->units[size_index].op2, op2, len2);
  }

  __strcmp_trace_ptr->size = min(__strcmp_trace_ptr->size + 1, STRCMP_MAX_CNT);

  return;
}

void strncmp_trace(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
  ...
  if (__strcmp_trace_ptr->units[size_index].match < prefix_bits) {
    __strcmp_trace_ptr->units[size_index].len1 = len1;
    __strcmp_trace_ptr->units[size_index].len2 = len2;
    __strcmp_trace_ptr->units[size_index].len3 = str_len;
    __strcmp_trace_ptr->units[size_index].match = prefix_bits;
    __builtin_memcpy(__strcmp_trace_ptr->units[size_index].op1, op1, len1);
    __builtin_memcpy(__strcmp_trace_ptr->units[size_index].op2, op2, len2);
  }
...
void strstr_trace(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
...
void memcmp_trace(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
...
void ptr_rtn_trace(int addr, u8* op1, u8* op2, unsigned long str_len) {
  if (__cmp_trace_ptr == NULL) {
    return;
  }
  if (addr > __branch_map_loc - 1) {
  ...
  if (op1 == NULL || op2 == NULL) return;
  int len1, len2;
  if ((len1 = area_is_valid(op1, STRCMP_TRACE_MAX_LEN)) <= 0 ||
        (len2 = area_is_valid(op2, STRCMP_TRACE_MAX_LEN)) <= 0) return;
...
void ptr_rtn_gcc_stdstring_stdstring_trace(int addr, u8 *stdstring1, u8 *stdstring2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  ptr_rtn_trace(addr, get_gcc_stdstring(stdstring1),
                    get_gcc_stdstring(stdstring2), str_len);
}

void ptr_rtn_gcc_stdstring_cstring_trace(int addr, u8 *stdstring, u8 *cstring, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  ptr_rtn_trace(addr, cstring, get_gcc_stdstring(stdstring), str_len);

}

void ptr_rtn_llvm_stdstring_cstring_trace(int addr, u8 *stdstring, u8 *cstring, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  ptr_rtn_trace(addr, cstring, get_llvm_stdstring(stdstring), str_len);
}

void ptr_rtn_llvm_stdstring_stdstring_trace(int addr, u8 *stdstring1, u8 *stdstring2, unsigned long str_len) {
  if (addr > __branch_map_loc - 1) {
    fprintf(stderr, "addr %d excceed __branch_map_loc %d\n", addr, __branch_map_loc - 1);
    abort();
  }
  ...
  ptr_rtn_trace(addr, get_llvm_stdstring(stdstring1), get_llvm_stdstring(stdstring2), str_len);
}
