...
void find_built_deps(aflcc_state_t *aflcc) {
  ...
  /* Check for Bulbasaur Full coverage pass (representative of all 4). */
  if ((ptr = find_object(aflcc, "bulbasaur-cov-full.so")) != NULL) {
...
/*
 * add_optimized_pcguard() — inject the Bulbasaur coverage pass plugin.
 *
 * Selects one of four bulbasaur-cov-*.so pass plugins based on BULBASAUR_INST_MODE
 * (or the legacy USE_FAST / USE_TRACE / USE_DEBUG env vars for backward
 * compatibility):
 *
 *   BULBASAUR_INST_MODE=FULL  (default) → bulbasaur-cov-full.so
 *     Full branch instrumentation, NoPrune=true. Used for coverage updates.
 *
 *   BULBASAUR_INST_MODE=FAST  → bulbasaur-cov-fast.so
 *     Edge-pruned fast path. Normal fuzzing loop; triggers Full re-execution
 *     only when a new edge is found.
 *
 *   BULBASAUR_INST_MODE=TRACE → bulbasaur-cov-trace.so
 *     Logs branch operand values to BULBASAUR_BRANCH_TRACE_PATH at runtime.
 *
 *   BULBASAUR_INST_MODE=DEBUG → bulbasaur-cov-debug.so
 *     Writes branch source-location CSV to BULBASAUR_BRANCH_LOC_PATH at compile
 *     time. Usually not run during fuzzing.
 *
 * Falls back to the native LLVM PCGUARD (fsanitize-coverage=trace-pc-guard)
 * when LLVM < 13, on Android, or when an allow/deny-list is in use.
 */
void add_optimized_pcguard(aflcc_state_t *aflcc) {
  ...
  if (aflcc->have_instr_list) {
    ...
    insert_param(aflcc, "-fexperimental-new-pass-manager");
    #endif
    /* Select Bulbasaur coverage pass via BULBASAUR_INST_MODE.
     * Legacy single-var env vars (USE_FAST, USE_TRACE, USE_DEBUG) are
     * still accepted for backward compatibility but deprecated.          */
    const char *bulbasaur_mode = getenv("BULBASAUR_INST_MODE");
    if (!bulbasaur_mode) {
      /* Legacy fallback */
      if      (getenv("USE_FAST"))  bulbasaur_mode = "FAST";
      else if (getenv("USE_TRACE")) bulbasaur_mode = "TRACE";
      else if (getenv("USE_DEBUG")) bulbasaur_mode = "DEBUG";
      else                          bulbasaur_mode = "FULL";
    }

    if (strcmp(bulbasaur_mode, "NATIVE") == 0) {
      /* Use LLVM's stable PCGuard pass when a project is not compatible with
       * one of the richer Bulbasaur IR passes.  The FuzzBench adapter uses
       * this as a build-time fallback and still supplies the normal AFL
       * compiler runtime. */
      insert_param(aflcc, "-fsanitize-coverage=trace-pc-guard");
      aflcc->instrument_mode = INSTRUMENT_LLVMNATIVE;
    } else if (strcmp(bulbasaur_mode, "FAST") == 0) {
      insert_object(aflcc, "bulbasaur-cov-fast.so", "-fpass-plugin=%s", 0);
      insert_param(aflcc, "-O0");
    } else if (strcmp(bulbasaur_mode, "TRACE") == 0) {
      insert_object(aflcc, "bulbasaur-cov-trace.so", "-fpass-plugin=%s", 0);
      insert_param(aflcc, "-O0");
    } else if (strcmp(bulbasaur_mode, "DEBUG") == 0) {
      insert_object(aflcc, "bulbasaur-cov-debug.so", "-fpass-plugin=%s", 0);
      insert_param(aflcc, "-g");
      insert_param(aflcc, "-O0");
    } else {
      /* Default: FULL */
      insert_object(aflcc, "bulbasaur-cov-full.so", "-fpass-plugin=%s", 0);
      insert_param(aflcc, "-O0");
    }
