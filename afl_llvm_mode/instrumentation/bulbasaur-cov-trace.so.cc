...
//===----------------------------------------------------------------------===//
//
// Bulbasaur: bulbasaur-cov-trace.so.cc  --  Branch Operand Trace Pass
// ===================================================================
//
// Records the raw operand values of every branch comparison to a CSV file
// at *runtime*. This is used for data-flow analysis and seed generation:
// knowing what values reach a branch helps the fuzzer mutate inputs that
// flip previously unseen branches.
//
// What it emits (set BULBASAUR_BRANCH_TRACE_PATH=<file.csv> before running):
//   <file.csv> – one row per branch execution:
//       branch_id, val_a, val_b   (for ICmp: the two operand integers;
//                                  for strcmp: lengths / hash of strings)
//
// Role in the pipeline:
//   - Run the target with BULBASAUR_BRANCH_TRACE_PATH pointing to a file.
//   - Load the trace CSV into the Bulbasaur seed synthesiser.
//   - The synthesiser uses (branch_id, val_a, val_b) tuples to derive
//     byte patterns that can satisfy branch conditions, then injects them
//     into the mutation engine.
//
// How it differs from the Full pass:
//   - NoPrune=false: edge pruning is enabled (same as Fast pass) for
//     consistent branch_ids with the Fast pass bitmap.
//   - The branch trace functions (trace_br8/16/32/64) write both operand
//     values to the output file rather than XOR-ing them into the bitmap.
//   - Uses shouldInstrumentBlockNoPrune() to ensure all branch sites emit
//     trace records, not just the pruned subset.
//
// Env vars (at runtime):
//   BULBASAUR_BRANCH_TRACE_PATH – path of the output CSV file. Must point to
//                             a writable file; pass errors out if missing.
//   AFL_DEBUG, AFL_QUIET    – verbosity controls.
//
// Built as: bulbasaur-cov-trace.so   (loaded via -fpass-plugin=)
// Plugin ID: "BulbasaurCovTrace"
//
//===----------------------------------------------------------------------===//
...
class BulbasaurCoverageTracePass
    : public PassInfoMixin<BulbasaurCoverageTracePass> {

 public:
  BulbasaurCoverageTracePass(
...
llvmGetPassPluginInfo() {
  ...
  return {LLVM_PLUGIN_API_VERSION, "BulbasaurCovTrace", "v1.0",
          ...
          [](PassBuilder &PB) {
            ...
            PB.registerOptimizerLastEPCallback(
                ...
                [](ModulePassManager &MPM, OptimizationLevel OL) {
                  ...
                  MPM.addPass(BulbasaurCoverageTracePass());
...
PreservedAnalyses BulbasaurCoverageTracePass::run(Module                &M,
                                                  ModuleAnalysisManager &MAM) {

  BulbasaurCoverageTracePass ModuleSancov(Options);
...
std::pair<Value *, Value *> BulbasaurCoverageTracePass::CreateSecStartEnd(
...
Function *BulbasaurCoverageTracePass::CreateInitCallsForSections(
    Module &M, const char *CtorName, const char *InitFunctionName, Type *Ty,
    const char *Section, const char *BranchSection, const char *BranchTypeSectionName,
    const char *EdgeToPredBranchSectionName) {
...
bool BulbasaurCoverageTracePass::instrumentModule(
  ...
  if ((isatty(2) && !getenv("AFL_QUIET")) || debug) {
    ...
    SAYF(cCYA "BulbasaurCov[Trace]" VERSION cRST "\n");
  ...
  const char *branch_loc_env = getenv("BULBASAUR_BRANCH_TRACE_PATH");
  if (branch_loc_env) {
    branch_loc_path = std::string(branch_loc_env);
    if (access(branch_loc_path.c_str(), F_OK) == -1) {
      fprintf(stderr, "Error: Branch location path %s is not valid\n", branch_loc_path.c_str());
      return true;
    }
  }
  ...
  if (!be_quiet) {
    ...
    if (!instr) {
      ...
      OKF("[SanitizerCoveragePCGUARDTrace] Instrumented %u locations with no collisions (%s mode) of which are "
          "%u handled and %u unhandled selects.",
          instr, modeline, selects, unhandled);
      OKF("[SanitizerCoveragePCGUARDTrace] Instrumented %u valid branches, calls cnt %u, gccstdstd cnt %u, "
...
bool BulbasaurCoverageTracePass::IsInterestingFunctionCall(CallInst *callInst, u8 favor_std, u8 favor_ptr_rtn) {
...
void BulbasaurCoverageTracePass::instrumentFunction(
  ...
  if (debug) {
    ...
    fprintf(stderr, "BulbasaurCov[Trace]: instrumenting %s in %s\n",
...
GlobalVariable *BulbasaurCoverageTracePass::CreateFunctionLocalArrayInSection(
...
GlobalVariable *BulbasaurCoverageTracePass::CreatePCArray(
...
void BulbasaurCoverageTracePass::CreateFunctionLocalArrays(
...
bool BulbasaurCoverageTracePass::InjectCoverage(
  ...
  for (auto &BB : F) {
    ...
    for (auto &IN : BB) {
      ...
      if ((callInst = dyn_cast<CallInst>(&IN))) {
        ...
        BulbasaurCoverageTracePass::SetNoSanitizeMetadata(Idx);
      ...
      if (!skip_next && (selectInst = dyn_cast<SelectInst>(&IN))) {
        ...
        BulbasaurCoverageTracePass::SetNoSanitizeMetadata(MapPtr);
        ...
        while (1) {
          ...
          if (!vector_cnt) {
            ...
            BulbasaurCoverageTracePass::SetNoSanitizeMetadata(CurLoc);
            ...
            BulbasaurCoverageTracePass::SetNoSanitizeMetadata(elementld);
          ...
          if (use_threadsafe_counters) {
            ...
            BulbasaurCoverageTracePass::SetNoSanitizeMetadata(Counter);
            ...
            BulbasaurCoverageTracePass::SetNoSanitizeMetadata(StoreCtx);
...
void BulbasaurCoverageTracePass::InjectTraceForSwitch(
...
void BulbasaurCoverageTracePass::InjectTraceForCmp(
...
void BulbasaurCoverageTracePass::InjectCoverageAtBlock(Function   &F,
                                                       BasicBlock &BB,
                                                       size_t      Idx,
                                                       bool        IsLeafFunc) {
...
std::string BulbasaurCoverageTracePass::getSectionName(
...
std::string BulbasaurCoverageTracePass::getSectionStart(
    const std::string &Section) const {
...
std::string BulbasaurCoverageTracePass::getSectionEnd(
    const std::string &Section) const {
...
bool BulbasaurCoverageTracePass::isSigned(Value *V0, Value *V1) {
...
void BulbasaurCoverageTracePass::TraceCmpBranch(
  ...
  if (branch_loc_path != "") {
    ...
    if (!branch_loc_file.is_open()) {
      ...
      branch_loc_file << "branch_offset,line,column\n";
    }
  }
  ...
  for (auto I : CmpTraceTargets) {
    ...
    if (CmpInst *CMP = dyn_cast<CmpInst>(I)) {
      ...
      if (branch_loc_path != "" && branch_loc_file.is_open()) {
        ...
        if (loc) {
          branch_loc_file << branch_offset << "," << loc->getLine() << "," << loc->getColumn() << "\n";
        } else {
          errs() << "Warning: CMP instruction at branch_offset " << branch_offset << " has no debug location\n";
        }
      }
...
void BulbasaurCoverageTracePass::TraceStrcmpBranch(
...
void BulbasaurCoverageTracePass::TraceNonTermStrcmp(Function &F, ArrayRef<Instruction *> StrcmpTraceTargetsNonTerminator) {
  PointerType *i8PtrTy = PointerType::get(Int8Ty, 0);
...
void BulbasaurCoverageTracePass::TraceSwitchBranch(Function &F, ArrayRef<Instruction *> SwitchTraceTargets, ArrayRef<ConstantInt *> case_val_list, std::vector<int> int_val_list) {
